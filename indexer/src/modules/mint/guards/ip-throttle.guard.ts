...
export class IPThrottlerGuard extends ThrottlerGuard {
  ...
  mintProgress: { progress: number, total: number } = { progress: 0, total: 0 };

  private readonly BASE_PENALTY_TIMEOUT = 10 * 1000; // 10 seconds base timeout
  private readonly PENALTY_TIMEOUT_INCREMENT = 20 * 1000; // 20 seconds increment
  ...
  constructor(
    ...
    // this.watchMintProgress();
    // setInterval(() => this.watchMintProgress(), 30000);
  }

  /**
   * Watches the minting progress for a specific collection
   */
  // async watchMintProgress() {
  //   const progress = await this.supabaseService.fetchMintProgress('call-data-comrades');
  ...
  private calculatePenaltyDuration(violations: number): number {
    ...
    if (violations === 1) return this.BASE_PENALTY_TIMEOUT;
    ...
    const additionalTime = (violations - 1) * this.PENALTY_TIMEOUT_INCREMENT;
    return this.BASE_PENALTY_TIMEOUT + additionalTime;
  }
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    try {
      ...
      if (error instanceof ThrottlerException) {
        ...
        // Reset violation count if it's been more than 30 minutes since last violation
        if (timeSinceLastViolation > 30 * 60 * 1000) {
      ...
      this.applyPenaltyTimeout(ip, this.BASE_PENALTY_TIMEOUT);

      const retryAfterSeconds = Math.ceil(this.BASE_PENALTY_TIMEOUT / 1000);
      response.header('Retry-After', String(retryAfterSeconds));
